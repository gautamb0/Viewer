...
namespace Leap.Unity.InputModule {
  public class LeapInputModule : BaseInputModule {
    ...
    [Tooltip("How long to disable the event system between clicks")] //gautam
    ...
    public override void Process() {
      if (Time.time - lastPointerUp < ActivationDelay) //gautam
          return;
      ...
      for (int whichPointer = 0; whichPointer < NumberOfPointers; whichPointer++) {
        ...
        if ((EnvironmentPointer)&&(pointerState[whichPointer] == pointerStates.OffCanvas)) {
          ...
          if (EnvironmentSpot.normal != Vector3.zero)
        ...
        if ((PointEvents[whichPointer] != null)) {
          ...
          if (PointEvents[whichPointer].pointerCurrentRaycast.gameObject != null && PointEvents[whichPointer].pointerCurrentRaycast.gameObject.GetComponentInParent<RTSEnabler>() != null)
                    {
                       // Debug.Log(PointEvents[whichPointer].pointerCurrentRaycast.gameObject);
                        PointEvents[whichPointer].pointerCurrentRaycast.gameObject.GetComponentInParent<RTSEnabler>().EnableRTS();
                    }
    ...
    private void UpdatePointer(int whichPointer, PointerEventData pointData) {
      if (EnvironmentPointer && pointerState[whichPointer] == pointerStates.OffCanvas) {
        Pointers[whichPointer].gameObject.SetActive(true);
                RaycastHit EnvironmentSpot;
                Vector3 ProjectionOrigin = Camera.main.transform.position + CurrentRotation * new Vector3(0.15f, -0.2f, 0f);
                Physics.Raycast(ProjectionOrigin, Pointers[whichPointer].transform.position - ProjectionOrigin, out EnvironmentSpot);
                if(EnvironmentSpot.transform!=null && EnvironmentSpot.transform.gameObject.GetComponentInParent<RTSEnabler>() != null)
                    EnvironmentSpot.transform.gameObject.GetComponentInParent<RTSEnabler>().EnableRTS();
